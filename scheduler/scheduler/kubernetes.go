...
func watchUnscheduledPods() (<-chan Pod, <-chan error) {
	...
	// Setup a Go routine that continually sends a watch request for pods
	// sends an 'ADDED' message to the pods channel
	go func() {
		for {
			resp, err := http.DefaultClient.Do(request)
			if err != nil {
...
func fit(pod *Pod) ([]Node, error) {
	...
	// Setup a map of Node name to resource use (e.g. CPU usage)
	...
	for _, p := range podList.Items {
		...
		// Retrieve the CPU cores of all the containers in a pod and sum the total and place in the ResourceUsage entry
		// for the Node
		for _, c := range p.Spec.Containers {
			if strings.HasSuffix(c.Resources.Requests["cpu"], "m") {
				...
				// Retrieve the ResourceUsage object
				ru := resourceUsage[p.Spec.NodeName]

				// Populate the int field with the number of cores
				ru.CPU += cores
			}
		}
	}
	...
	// Total CPU required for all the unscheduled pods
	...
	// Iterate through the nodes and only append nodes that meet the free CPU requirement
	for _, node := range nodeList.Items {
		var allocatableCores int
		var err error
		// Populate allocatableCores from Node if suffix present otherwise do another calculation to set it
	...
	// If no nodes have free space then emit a Kubernetes event
	if len(nodes) == 0 {
		// Emit a Kubernetes event that the Pod wasn't scheduled successfully.
...
// Bind the pod to the specific Node
func bind(pod *Pod, node Node) error {
	...
	// Send the request to bind a pod to specific Node
	request := &http.Request{
		Body:          io.NopCloser(body),
		ContentLength: int64(body.Len()),
		Header:        make(http.Header),
